...
## [1.1.1-corporate.1] - 2026-08-26

### Updated
- Corporate wrapper policy mirrors only `specs/<feature>/spec.md` to Confluence.
- Added automatic `before_plan` hook command `background-spec-sync`.
- Added local `confluence-mapping.json` and `confluence-sync-trace.json` contract.
- Required Confluence mirror pages to be rendered in Russian while preserving technical identifiers and local source files.
