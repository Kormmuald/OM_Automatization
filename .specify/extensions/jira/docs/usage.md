...
1. **Write Jira text in Russian**: summaries, descriptions, comments, and generated progress notes should be readable in Russian. Keep technical identifiers such as issue keys, paths, field IDs, status tokens, URLs, code identifiers, and task IDs unchanged.
2. **Run discover-fields first**: Always discover custom fields before configuring them
3. **Use local config for testing**: Keep project config clean, use local overrides for experiments
4. **Commit mapping files**: Add `.specify/jira-mapping.json` to git for team visibility
5. **Sync regularly**: Run sync-status after each task completion for accurate tracking
6. **Version control config**: Commit `jira-config.yml` for consistent team workflows
