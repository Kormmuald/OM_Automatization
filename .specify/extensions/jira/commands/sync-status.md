...
- comments
- progress/status text appended to descriptions
- generated task or epic summary fragments
...
Do not send English boilerplate such as `Task completed locally via spec-kit`, `Progress`, `Updated Tasks`, or `Skipped` to Jira. Trace/log JSON may keep stable English machine tokens.
...
#     # Tool: issue_update
#     # Parameters: { "issue_key": "$task_key", "comment": "Задача завершена локально через Spec Kit." }
#   else
...
# Tool: issue_update
# Parameters: { "issue_key": "$epic_key", "description": "...\\n\\nПрогресс: $completion_pct% ($completed_count/$total_tasks задач завершено)" }
```
