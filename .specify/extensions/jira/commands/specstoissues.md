...
### Jira-Facing Language Policy

Every text value written into Jira for human reading MUST be in Russian:

- `summary`
- `description`
- comments
- generated progress/status text
- generated labels inside descriptions, such as task lists and phase metadata

Preserve technical identifiers as-is: issue keys, file paths, field IDs, status tokens, command names, requirement IDs, URLs, code identifiers, and task IDs like `T001`.

Use source artifact text directly when it is already Russian. When source artifact headings or task text are in English, translate generated Jira-facing prose around them into Russian and, where practical, translate the human-readable heading/task text while keeping identifiers and code paths intact. Do not send English boilerplate such as `Phase from spec`, `Task from spec`, `Status in spec-kit`, `Progress`, or `Tasks` to Jira.
...
Parameters:
  ...
  - summary: {phase_name}
  - description: "Фаза из спеки: {spec_name}\n\nЗадачи:\n- T001: ...\n- T002: ..."
...
Parameters:
  ...
  - description: "Задача из спеки: {spec_name}\nФаза: {phase_name}\nСтатус в Spec Kit: {task_status}"
