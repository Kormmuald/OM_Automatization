...
metadata:
  author: "ismaelJimenez/spec-kit-verify"
  source: ".specify/extensions/verify/commands/verify.md"
scripts:
...
**Report Language**: Write the user-facing report in Russian. Translate headings, table column names, summaries, recommendations, metrics labels, next actions, and the remediation question to Russian. Preserve technical identifiers, command names, file paths, task IDs, requirement IDs, severity tokens (`CRITICAL`, `HIGH`, `MEDIUM`, `LOW`), and stable finding IDs (`A1`, `B1`, `C1`...) as-is.
