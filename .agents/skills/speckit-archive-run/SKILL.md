...
metadata:
  author: "stn1slv/spec-kit-archive"
  source: ".specify/extensions/archive/commands/archive.md"
...
- `## Clarifications` / `## Уточнения`, if the template carries one, per **Allowed Sources** — that heading is never carried into main memory in any seeding path;
...
- Do **not** extract the `## Clarifications` / `## Уточнения` session log — its decisions already live in the sections above (see Allowed Sources)
...
- Per report file, the header fields only: bug ID and title (the report's first line), Type, Severity, and `**Status**` / `**Статус**`, whatever its value. Real reports vary: a field may be missing (record it as absent, never infer it), a status may say `Fixed`, `Исправлено`, or anything else, and the root-cause section may not exist.
- Per report file, the `## Root Cause Analysis` / `## Анализ первопричины` section (or none) — solely for the agent file's Known Issues merge in 5.3
...
- Max 3 unresolved `NEEDS CLARIFICATION` / `ТРЕБУЕТ УТОЧНЕНИЯ` markers in output — beyond that, make reasonable defaults and note them in the report.
...
2. If found, update the sections below, creating any that are missing. The section set is defined **here**, not by a template: recent spec-kit versions ship no agent-file template (an older project may still carry one in `.specify/templates/`, in which case follow its layout for these sections). If the file contains a tool-managed marker block (such as `<!-- SPECKIT ... -->` markers), **never write inside the markers** — the CLI overwrites that region; place these sections outside it.
   ...
   **"Known Issues & Gotchas"** — if `research.md` exists in the feature, extract any gotchas/issues; if `bugs/` reports carry a `## Root Cause Analysis` / `## Анализ первопричины` (the one content read Allowed Sources permits from them), turn each into an entry titled with the bug ID and short title. Merge both kinds using the standard format:
   ```markdown
   ### ⚠️ [Issue Title]
...
In the feature's `spec.md` and `plan.md` files (inside `FEATURE_DIR`, **not** in memory), check for a status metadata field in the document header (typically in the first 10 lines, e.g., `**Status**: Draft` or `**Статус**: Черновик`).

If found and the value is `Draft` or `Черновик`, update it to the completed value in the same language:
- `**Status**: Draft` → `**Status**: Completed`
- `**Статус**: Черновик` → `**Статус**: Завершено`

This marks the feature specification as finalized after merge. Do not change other status values (e.g., `In Progress`, `Blocked`, `В работе`, `Заблокировано`) — only `Draft` → `Completed` and `Черновик` → `Завершено`.
...
**Report Language**: Write the user-facing report in Russian. Translate headings, table column names, summaries, recommendations, metrics labels, next actions, and explanatory text to Russian. Preserve technical identifiers, command names, file paths, task IDs, requirement IDs, section names quoted from source artifacts, status tokens (`Draft`, `Completed`, `Черновик`, `Завершено`, `NEEDS CLARIFICATION`, `ТРЕБУЕТ УТОЧНЕНИЯ`), markers (`RETIRED:`), and stable source refs (`[Source: ...]`) as-is.
...
- For each executable mandatory hook, emit `EXECUTE_COMMAND: /{command}` and then actually invoke the hook before recommendations are finalized.
- Pass the resolved absolute `FEATURE_DIR` as the hook input so post-archival integrations act on exactly the feature spec that was just archived.
- If a post-archival hook fails, report the failure in recommendations and keep the archival result intact.
- If no hooks are registered or the file does not exist, skip silently
...
- Feature spec `**Status**: Draft` updated to `Completed`, or `**Статус**: Черновик` updated to `Завершено` (if applicable).
- Conflicts either resolved (with user input) or marked with `NEEDS CLARIFICATION` / `ТРЕБУЕТ УТОЧНЕНИЯ` (max 3). This covers the 2.2 and 2.3 kinds; an unresolved **constitution** conflict takes the third disposition instead — the item is withheld and named in the report, since an item that was never written has nowhere to carry a marker.
